...
import urllib3, os, cre, json, client, sqlite3, os, time
...
class Vklass:
	...
	def login_from_file(self):
		if os.path.exists(self.configuration_file):
			...
			try:
				...
				client.log("Could not log in to %s with the credentials in %s" %
				           (self.base_url, self.configuration_file))

	def create_tables(self):
		client.log("Creating tables")
		try:
			self.cursor.execute(
			 "CREATE TABLE users (id integer primary key autoincrement, profile_id integer, profile_id2 text, name text, class text, age text, msn text, email text, cellphone_number text, school text, avatar_filename text)"
			)
			self.cursor.execute(
			 "CREATE TABLE chat (id integer primary key autoincrement, from_id text, to_id text, message text, time integer)"
			)
			self.dbc.commit()
		except:
	...
	def login(self, username, password):
		...
		vklass_opener.addheaders = [(
		 'User-agent',
		 'Mozilla/5.0 (X11; U; Linux x86_64; en-US; rv:1.9.2.10) Gecko/20100922 Ubuntu/10.10 (maverick) Firefox/3.6.11'
		)]
		urllib2.install_opener(vklass_opener)

		index = urllib2.urlopen(self.base_url).read()
		eventvalidation = cre.between(
		 '<input type="hidden" name="__EVENTVALIDATION" id="__EVENTVALIDATION" value="',
		 '" />', index)
		viewstate = cre.between(
		...
		login = urllib2.urlopen(self.base_url + '/login.aspx?cookieCheck=true',
		                        login_data)
		if 'title="Logga ut' in urllib2.urlopen("%s/default.aspx" %
		                                        self.base_url).read():
	...
	def stay_logged_in(self):
		if 'title="Logga ut' not in urllib2.urlopen("%s/default.aspx" %
		                                            self.base_url).read():
	...
	def profile_info(self, profile_id):
		...
		if dbresult:
			...
			try:
				...
				profile_id = int(
				 cre.between('href="Guestbook.aspx\?id=', '"', profile_data))
				profile_id2 = cre.between(
				 'frameborder="0" src="https://user.vklass.se/presentation/', '"',
				 profile_data)
				profile_name = cre.between(
				 '<li><span id="ctl00_ContentPlaceHolder2_nameLabel">Namn: ',
				 '</span></li>', profile_data)
				profile_class = cre.between(
				 '<li><span id="ctl00_ContentPlaceHolder2_classLabel">Klass: ',
				 '</span></li>', profile_data)
				age = cre.between('<li><span id="ctl00_ContentPlaceHolder2_ageLabel">',
				                  '</span></li>', profile_data)
				msn = cre.between(
				 '<li><span id="ctl00_ContentPlaceHolder2_msnLabel">MSN: ', '</span></li>',
				 profile_data)
				email = cre.between(
				 '<li><span id="ctl00_ContentPlaceHolder2_mailLabel">Email: ',
				 '</span></li>', profile_data)
				cellphone_number = cre.between(
				 '<li><span id="ctl00_ContentPlaceHolder2_mobileLabel">Mobil: ',
				 '</span></li>', profile_data)
				school = cre.between('<li>Skola: ', '<', profile_data)
				avatar_filename = cre.between('https://user.vklass.se/photo/large/',
				                              '" target="', profile_data)

				self.cursor.execute(
				 "INSERT INTO users (profile_id, profile_id2, name, class, age, msn, email, cellphone_number, school, avatar_filename) VALUES(%i, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')"
				 % (profile_id, profile_id2, profile_name, profile_class, age, msn, email,
				    cellphone_number, school, avatar_filename))
				self.dbc.commit()
				client.ensure_dir(self.profile_picture_dir)
				if avatar_filename:
					client.download('%s/photo/large/%s' %
					                (self.base_url, avatar_filename,
					                 self.profile_picture_dir + "/" + avatar_filename))
				info = {
	...
	def users_online(self):
		users_online = json.loads(
		 urllib2.urlopen("%s/handler/usersOnline.ashx?action=friendsonline" %
		                 self.base_url).read())['users']
		return users_online
	...
	def send_chat(self, to, message):
		client.log("Sending message: %s, to: %s" %
		           (message, self.info_by_id2(to)['UserName']))
		...
		self.cursor.execute(
		 "INSERT INTO chat (from_id, to_id, message, time) VALUES('%s', '%s', '%s', %i)"
		 % ('0', to, message.replace("'", "''"), int(time.time())))
		self.dbc.commit()
	...
	def get_chat(self):
		...
		for message in json.loads(chat_messages_raw)['items']:
			...
			self.cursor.execute(
			 "INSERT INTO chat (from_id, to_id, message, time) VALUES('%s', '%s', '%s', %i)"
			 % (sender.replace("'", "''"), '0', message.replace(
			  "'", "''"), int(time.time())))
			self.dbc.commit()
	...
	def news_listing(self):
		...
		dirty_news_ids = cre.all_between(
		 '<a id="ctl00_ContentPlaceHolder2_newsRepeater_ctl', "&amp;", html)
		clean_news_ids = []
	...
	def news(self, news_id):
		...
		html = urllib2.urlopen("https://www.vklass.se/SchoolNews.aspx?id=%i" %
		                       news_id).read()
		news['body'] = cre.prettify(
		 cre.between('<span id="ctl00_ContentPlaceHolder2_newsShortLabel">',
		             '</span>', html))
		news['title'] = cre.prettify(
		 cre.between('<span id="ctl00_ContentPlaceHolder2_nameLabel">', '</span>',
		             html))
		attached_file_html_chunk = cre.between(
		 '<a id="ctl00_ContentPlaceHolder2_NewsAttachmentLink" href="', '</a>', html)
	...
	def exam_statistics(self, exam_id):
		...
		html = urllib2.urlopen("%s/ExamStatistics.aspx?id=%i" %
		                       (self.base_url, exam_id)).read()
		...
		for error_message in critical_error_messages:
			...
			exam = {
			 "possible_grades": None,
			 "exam_participants": None,
			 "course_participants": None,
			 "grades": None
			}
			exam['name'] = cre.between(
			 '<span id="ctl00_ContentPlaceHolder2_nameLabel">', '</span>', html)
			exam['course'] = cre.between(
			 '<span id="ctl00_ContentPlaceHolder2_courseLabel">', '</span>', html)
			exam['type'] = cre.between(
			 '<span id="ctl00_ContentPlaceHolder2_typeLabel">', '</span>',
			 html).replace("  ", "")
			exam['date'] = cre.between(
			 '<span id="ctl00_ContentPlaceHolder2_dateLabel">', '</span>', html)
			if "ej statistik" not in html:
				exam['possible_grades'] = cre.between(
				 '<span id="ctl00_ContentPlaceHolder2_gradingLabel">', '</span>',
				 html).split("/")
				exam['exam_participants'] = cre.between(
				 '<span id="ctl00_ContentPlaceHolder2_numberLabel">', '</span>',
				 html).split(" ")[0]
				exam['course_participants'] = cre.between(
				 '<span id="ctl00_ContentPlaceHolder2_numberLabel">', '</span>',
				 html).split(" ")[2]
				exam['grades'] = {}
				for offset in range(len(exam['possible_grades'])):
					exam['grades'][exam['possible_grades'][offset]] = cre.between(
					 '<span id="ctl00_ContentPlaceHolder2_infoLabel">', '</span>',
					 html).replace(" ", "").split(":")[1 + offset].split("<")[0]
	...
	def class_calendar_exam_ids(self):
		...
		ids = [
		 int(id)
		 for id in cre.all_between('<a href="ExamStatistics\\.aspx\\?id=', '&', html)
		]
	...
	def class_events(self):
		events = []
		html = urllib2.urlopen("%s/ClassCalendar.aspx?id=%s" %
		                       (self.base_url, self.class_uid())).read()
		event_chunks = cre.all_between(
		 '<span id="ctl00_ContentPlaceHolder2_monthsRepeater_ctl.._eventsRepeater_ctl.._topicLabel">Klassh..ndelse: ',
		 '</span></dd>', html)
		for trash in event_chunks:
			event = {
			 "name": trash.split("</span>")[0],
			 "description": trash.split('">Beskrivning: ')[-1]
			}
			events.append(event)
	...
	def time_summary(self):
		...
		trash = cre.between(
		 '<span id="ctl00_ContentPlaceHolder2_attendanceMinutesLabel">', '</span>',
		 html).split(" ")
		time = {"present": {}, "absent": {}}
		...
		time['absent'][
		 'minutes'] = time['scheduled_minutes'] - time['present']['minutes']
	...
	def current_schedule(self):
		...
		for chunk in cre.all_between('<div class="LessonInfoContainer"', '</td>',
		                             html):
			...
			day_replacements = [["M", "monday"], ["Ti",
			                                      "tuesday"], ["Ons", "wednesday"],
			                    ["To", "thursday"], ["F", "friday"]]
	...
	def courses(self):
		...
		courses = cre.all_between(
		 '<td class="kurs"><a id="ctl00_ContentPlaceHolder2_StudentRepeater_ctl.._courseLink" href="Course.aspx\?id=.{5}">',
		 '</a>', html)
	...
	def food_menu(self):
		...
		days = cre.all_between(
		 '<strong><span id="ctl00_ContentPlaceHolder2_lunchRepeater_ctl.._dayLabel">',
		 '</span></strong>', html)
		meals = cre.all_between(
		 '<span id="ctl00_ContentPlaceHolder2_lunchRepeater_ctl.._dayMenuLabel">',
		 '</span>',
		 html.replace("<br />", "").replace("\n", "").replace("\r", ""))

		for id in range(len(days)):
			for day in [["M", "monday"], ["Ti", "tuesday"], ["Ons", "wednesday"],
			            ["To", "thursday"], ["F", "friday"]]:
	...
	def message(self, message_id):
		html = urllib2.urlopen("%s/Messaging/MessageRead.aspx?id=%i" %
		                       (self.base_url, message_id)).read()
		message = {"attachments": [], "participants": [], "posts": []}
		message['owner'] = self.username
		message['body'] = cre.prettify(
		 cre.all_between(
		  '<span id="ctl00_ContentPlaceHolder2_postRepeater_ctl.._textLabel">',
		  '</span>', html)[-1])
		message['title'] = cre.prettify(
		 cre.between('<span id="ctl00_ContentPlaceHolder2_topicLabel">', '</span>',
		             html))
		message['created'] = cre.prettify(
		 cre.between('<span id="ctl00_ContentPlaceHolder2_dateLabel">', '</span>',
		             html))
		trash = cre.all_between(
		 '<span id="ctl00_ContentPlaceHolder2_postRepeater_ctl.._posterLabel">',
		 '</p>', html)
		del trash[-1]
		for trash in trash:
			...
			date = cre.between(
			 '<span id="ctl00_ContentPlaceHolder2_postRepeater_ctl.._Label1">',
			 '</span>', trash)
			post = {"username": username, "date": date, "body": body}
			message['posts'].append(post)

		trash = cre.between(
		 '<a id="ctl00_ContentPlaceHolder2_creatorLink" href="/User.aspx\?id=',
		 '</a></h3>', html).split('">')
		message['creator'] = {'user_id': int(trash[0]), "name": trash[1]}
		participants_dirty_html_chunks = cre.between('<h3>Deltagare: ', '</h3>',
		                                             html).split(', ')
		for dirty_participant_chunk in participants_dirty_html_chunks:
			user_id = int(
			 cre.between('<a href="/User.aspx\?id=', '">', dirty_participant_chunk))
		...
		dirty_attachment_html_chunks = cre.all_between('_AttachmentLink" href="',
		                                               '</a></li>', html)
	...
	def first_message_ids(self):
		return [
		 int(x) for x in cre.all_between(
		  'href="MessageRead.aspx\?id=', '&amp;',
		  urllib2.urlopen(self.base_url + "/Messaging/Messages.aspx").read())
		]
	...
	def first_messages(self):
		...
		for message_id in self.first_message_ids():
			client.push("Fetching message: %i [%i/%i]" %
			            (message_id, i + 1, len(message_ids)))
