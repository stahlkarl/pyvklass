...
class Notifier:
	...
	def conf(self):
		...
		return json.loads(
		 client.load(self.configuration_filename).replace("\r",
		                                                  "").replace("\n", ""))
	...
	def loop(self):
		...
		pygmail.login(self.conf()['accounts']['mail']['username'],
		              self.conf()['accounts']['mail']['password'])

		for account in self.conf()['accounts']['vklass']:
			client.log("Spawning vklass session with username: %s, email: %s" %
			...
			for session in self.sessions:
				for message in session.first_messages():
					if json.dumps(message) not in self.saved_messages():
						...
						client.append(self.conf()['files']['saved_messages'],
						              json.dumps(message) + "\n")
			time.sleep(120)
